...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Function to handle adding/removing active class
    const handleChangelogClick = (changelog) => {
        changelog.classList.toggle('active');

        // Remove active class from other changelog
        changelogs.forEach(other => {
            if (other !== changelog) {
                other.classList.remove('active');
            }
        });
    ...
    // Add click event listeners for changelog
    changelogs.forEach(changelog => {
        changelog.addEventListener('click', function() {
            handleChangelogClick(changelog);
        });
    });
